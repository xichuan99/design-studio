"""Scheduled reconcile worker for credit purchases.

Scans pending credit purchases that are older than a threshold and checks their
...
async def _reconcile_one(purchase_id: UUID) -> str:
    ...
    from app.services.credit_payment_service import process_webhook_event
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            logger.error(
                "credit.reconcile.provider_error purchase_id=%s detail=%s",
                purchase_id,
                exc.detail,
            )
            return "error"
        ...
        try:
            ...
            logger.error(
                "credit.reconcile.fulfill_error purchase_id=%s detail=%s",
                purchase_id,
                exc.detail,
            )
            return "error"
        ...
        logger.info(
            "credit.reconcile.outcome purchase_id=%s status=%s",
            purchase_id,
            internal_status,
        )
...
@shared_task(
    bind=True,
    name="app.workers.credit_reconcile.reconcile_pending_credit_purchases",
    max_retries=0,
    ignore_result=True,
)
def reconcile_pending_credit_purchases(self) -> None:  # type: ignore[override]
    """
    Celery beat task: check pending credit purchases against Midtrans
    ...
    logger.info("credit.reconcile.start")
    ...
    async def _run():
        async with AsyncSessionLocal() as db:
            result = await db.execute(
                ...
                .where(
                    ...
                    CreditPurchase.provider == "midtrans",
                    CreditPurchase.created_at <= threshold,
                )
                .order_by(CreditPurchase.created_at)
                .limit(RECONCILE_BATCH_SIZE)
            )
            purchase_ids = [row[0] for row in result.all()]

        logger.info("credit.reconcile.candidates count=%d", len(purchase_ids))
        ...
        logger.info(
            "credit.reconcile.done fulfilled=%d failed=%d expired=%d "
