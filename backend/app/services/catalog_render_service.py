...
async def start_catalog_render_job(
    *,
    ...
    if estimated_cost > 0 and current_user.credits_remaining < estimated_cost:
    ...
    if is_new_job:
        ...
        try:
            if use_celery:
                ...
                background_coro = _tracked_run_ai_tool_job(str(job.id))
                asyncio.create_task(background_coro)
        except Exception as exc:
            if background_coro is not None:
                background_coro.close()
            logger.exception("Failed to dispatch catalog render job", extra={"job_id": str(job.id)})
            if estimated_cost > 0:
                await log_credit_change(
                    db,
                    current_user,
                    estimated_cost,
                    "Refund: dispatch catalog render gagal",
                )
                payload = dict(job.payload_json or {})
            ...
            job.status = "failed"
            job.error_message = "Gagal memulai render katalog."
            job.phase_message = "Render katalog gagal dimulai"
            job.finished_at = datetime.now(timezone.utc)
            db.add(job)
            await db.commit()
            await db.refresh(job)
            raise RuntimeError("Gagal memulai render katalog.") from exc
