...
class CreditTransactionResponse(BaseModel):
    ...
    amount: int = Field(
        ...,
        description="Amount of credits added (positive) or deducted (negative)",
        json_schema_extra={"example": -1},
    )
    balance_after: int = Field(
        ...,
        description="User's credit balance after the transaction",
        json_schema_extra={"example": 9},
    )
    description: str = Field(
        ...,
        description="Description of the transaction",
        json_schema_extra={"example": "Hapus background"},
    )
    created_at: datetime = Field(
        ...,
        description="Timestamp of the transaction",
        json_schema_extra={"example": "2024-03-15T12:05:00Z"},
    )
