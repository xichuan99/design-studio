...
def test_operator_summary_returns_paid_beta_snapshot(monkeypatch):
    ...
    try:
        with (
            ...
            patch("app.api.internal_metrics._signups_by_invite_source", new_callable=AsyncMock) as signups_by_source,
            patch("app.api.internal_metrics._count_repeat_purchasers", new_callable=AsyncMock) as repeat_purchasers,
            ...
            scalar_int.side_effect = [12, 3, 8, 320, 400, 80, 149000, 149000, 6, 3, 2, 4]
            ...
            signups_by_source.return_value = {"email_allowlist": 2, "credentials": 1}
        ...
        assert data["users"]["signups_by_invite_source_7d"]["email_allowlist"] == 2
        ...
        assert data["payments"]["credit_by_status"]["paid"] == 2
        assert data["payments"]["credit_revenue_30d_idr"] == 149000
        assert data["payments"]["credit_paid_30d"] == 3
        assert data["payments"]["credit_repeat_purchasers_30d"] == 1
