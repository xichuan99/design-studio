...
@pytest.mark.asyncio
@patch("app.services.ai_usage_service.is_usage_refunded", new_callable=AsyncMock)
...
async def test_refund_ai_tool_job_if_needed_refunds_once(
    ...
    mock_is_usage_refunded,
):
    mock_is_usage_refunded.return_value = False
    session = AsyncMock()
...
@patch("app.services.credit_service.log_credit_change", new_callable=AsyncMock)
@patch("app.services.ai_usage_service.update_usage_for_job", new_callable=AsyncMock)
async def test_refund_ai_tool_job_if_needed_marks_ledger_when_payload_already_refunded(
    mock_update_usage_for_job,
    mock_log_credit_change,
    mock_is_usage_refunded,
):
...
@patch("app.services.credit_service.log_credit_change", new_callable=AsyncMock)
@patch("app.services.ai_usage_service.is_usage_refunded", new_callable=AsyncMock)
async def test_refund_ai_tool_job_if_needed_skips_when_ledger_already_refunded(
    mock_is_usage_refunded,
    mock_log_credit_change,
):
    mock_is_usage_refunded.return_value = True
    session = AsyncMock()
    session.add = MagicMock()
    session.get = AsyncMock(return_value=SimpleNamespace(id="user-1"))
