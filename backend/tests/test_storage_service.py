...
@patch('app.services.storage_service.settings')
@patch('app.services.storage_service._get_s3_client')
async def test_upload_image_s3_fallback_aws_url(mock_get_s3, mock_settings, image_bytes):
    mock_settings.S3_ACCESS_KEY = "valid-key"
    mock_settings.S3_SECRET_KEY = "valid-secret"
    mock_settings.S3_BUCKET = "test-bucket"
    mock_settings.S3_PUBLIC_URL = ""
...
@patch('os.makedirs')
async def test_upload_image_local_fallback_no_credentials(mock_makedirs, mock_file, mock_settings, image_bytes):
    mock_settings.S3_ACCESS_KEY = ""
    mock_settings.BACKEND_BASE_URL = "http://localhost:8000"

    result = await upload_image(image_bytes, key="test/file.png", content_type="image/png")
...
@patch('builtins.open', new_callable=mock_open)
@patch('os.makedirs')
async def test_upload_image_local_fallback_s3_error(mock_makedirs, mock_file, mock_get_s3, mock_settings, image_bytes):
    mock_settings.S3_ACCESS_KEY = "valid-key"
    mock_settings.S3_SECRET_KEY = "valid-secret"
    mock_settings.BACKEND_BASE_URL = "http://localhost:8000"
...
async def test_download_image(mock_client_cls):
    ...
    mock_client_instance.get.assert_called_once_with("http://image.url", follow_redirects=True)
